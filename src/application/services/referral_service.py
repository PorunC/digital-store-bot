...
class ReferralApplicationService:
    ...
    async def create_referral(
        ...
        async with self.unit_of_work:
            ...
            referrer = await self._get_user_repository().get_by_id(referrer_id)
            if not referrer:
                raise ValueError(f"Referrer with ID {referrer_id} not found")

            referred_user = await self._get_user_repository().get_by_id(referred_user_id)
            if not referred_user:
                raise ValueError(f"Referred user with ID {referred_user_id} not found")

            # Check if user already has a referrer
            existing_referral = await self._get_referral_repository().find_by_referred_user(
            ...
            referral = await self._get_referral_repository().add(referral)
            
            # Update referrer's referral count
            referrer.increment_referrals()
            await self._get_user_repository().update(referrer)
    ...
    async def activate_referral(
        ...
        async with self.unit_of_work:
            referral = await self._get_referral_repository().find_by_referred_user(
                uuid.UUID(referred_user_id)
            )
            ...
            referrer = await self._get_user_repository().get_by_id(str(referral.referrer_id))
            if referrer:
            ...
            referral = await self._get_referral_repository().update(referral)
            await self._publish_events(referral)
            await self.unit_of_work.commit()
            return referral
    ...
    async def get_referral_by_id(self, referral_id: str) -> Optional[Referral]:
        """Get referral by ID."""
        async with self.unit_of_work:
            return await self._get_referral_repository().get_by_id(referral_id)

    async def get_user_referrals(self, referrer_id: str) -> List[Referral]:
        """Get all referrals for a user."""
        async with self.unit_of_work:
            return await self._get_referral_repository().find_by_referrer(uuid.UUID(referrer_id))

    async def get_active_referrals(self, referrer_id: str) -> List[Referral]:
        """Get active referrals for a user."""
        async with self.unit_of_work:
            return await self._get_referral_repository().find_active_referrals(uuid.UUID(referrer_id))

    async def get_user_referrer(self, user_id: str) -> Optional[Referral]:
        """Get the referral record for a user (who referred them)."""
        async with self.unit_of_work:
            return await self._get_referral_repository().find_by_referred_user(uuid.UUID(user_id))
    ...
    async def process_pending_rewards(
        ...
        async with self.unit_of_work:
            pending_referrals = await self._get_referral_repository().find_pending_rewards(reward_type)
    ...
    async def deactivate_referral(
        ...
        async with self.unit_of_work:
            referral = await self._get_referral_repository().get_by_id(referral_id)
            if not referral:
    ...
    async def get_referral_statistics(self, referrer_id: str) -> dict:
        ...
        async with self.unit_of_work:
            return await self._get_referral_repository().get_referral_stats(uuid.UUID(referrer_id))
    ...
    async def get_top_referrers(self, limit: int = 10) -> List[dict]:
        ...
        async with self.unit_of_work:
            ...
            all_referrals = await self._get_referral_repository().get_all()
            ...
            for stats in top_referrers:
                user = await self._get_user_repository().get_by_id(stats["referrer_id"])
                if user:
                    stats["user_info"] = {
